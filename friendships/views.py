...
from friendships.task import send_realtime_event
...
class SendRequest(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        to_user = get_object_or_404(Account, id=to_user_id)

        if request.user != to_user:
            friend_request_exists = Request.objects.filter(
            ...
            if not friend_request_exists:
                ...
                friend_request.save()
                send_realtime_event.delay(to_user_id)
                print("Lời mời kết bạn đã được gửi")
                return Response(
...
def ListFriends(request, user_id):
    ...
    friends_from_received_requests = [
        request.requester for request in received_requests
    ]
    ...
    friends_data = [
        {"id": friend.id, "name": friend.username} for friend in unique_friends
    ]
...
def Friends_list(request, user_id):
    ...
    # JSON LIST FRIEND
    friends_data = [
        {"id": friend.id, "name": friend.username} for friend in friend_accounts
    ]
